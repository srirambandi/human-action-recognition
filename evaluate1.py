...
flow_test_path = ["../data/binary/Misc/f_test"+str(i)+".p" for i in range(6)]
rgb_test_path = ["../data/binary/Misc/r_test"+str(i)+".p" for i in range(6)]
label_test_path = ["../data/binary/Misc/l_test"+str(i)+".p" for i in range(6)]
...
for t in range(1):
        rgb_model = Inception_Inflated3d(include_top=False, weights='rgb_imagenet_and_kinetics', input_shape=(None, 7, 7, 832), classes=2)
        rgb_model.load_weights("/home/prathosh/ram/keras-kinetics-i3d/data/binary/Misc/rgb"+str(t)+".h5")
        flow_model = Inception_Inflated3d(include_top=False, weights='flow_imagenet_and_kinetics', input_shape=(None, 7, 7, 832), classes=2)
        flow_model.load_weights("/home/prathosh/ram/keras-kinetics-i3d/data/binary/Misc/flow"+str(t)+".h5")
        count = 0
