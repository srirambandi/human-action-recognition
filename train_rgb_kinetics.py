...
# -*- coding: utf-8 -*-
"""
Created on Sun Aug 12 14:43:33 2018
...
WEIGHTS_PATH = {
    'rgb_kinetics_only' : '/home/mech/btech/me1130654/keras-kinetics-i3d/weights/i3d_inception_rgb_kinetics_only.h5',
    'flow_kinetics_only' : '/home/mech/btech/me1130654/keras-kinetics-i3d/weights/i3d_inception_flow_kinetics_only.h5',
    'rgb_imagenet_and_kinetics' : '/home/mech/btech/me1130654/keras-kinetics-i3d/weights/i3d_inception_rgb_imagenet_and_kinetics.h5',
    'flow_imagenet_and_kinetics' : '/home/mech/btech/me1130654/keras-kinetics-i3d/weights/i3d_inception_flow_imagenet_and_kinetics.h5'
}

# path to pretrained models with no top (no classification layer)
WEIGHTS_PATH_NO_TOP = {
    'rgb_kinetics_only' : '/home/mech/btech/me1130654/keras-kinetics-i3d/weights/i3d_inception_rgb_kinetics_only_no_top.h5',
    'flow_kinetics_only' : '/home/mech/btech/me1130654/keras-kinetics-i3d/weights/i3d_inception_flow_kinetics_only_no_top.h5',
    'rgb_imagenet_and_kinetics' : '/home/mech/btech/me1130654/keras-kinetics-i3d/weights/i3d_inception_rgb_imagenet_and_kinetics_no_top.h5',
    'flow_imagenet_and_kinetics' : '/home/mech/btech/me1130654/keras-kinetics-i3d/weights/i3d_inception_flow_imagenet_and_kinetics_no_top.h5'
}
...
def Inception_Inflated3d(include_top=True,
    ...
    # Mixed 5c
    branch_0 = conv3d_bn(x, 384, 1, 1, 1, padding='same', name='Conv3d_5c_0a_1x1_Trainable')

    branch_1 = conv3d_bn(x, 192, 1, 1, 1, padding='same', name='Conv3d_5c_1a_1x1_Trainable')
    branch_1 = conv3d_bn(branch_1, 384, 3, 3, 3, padding='same', name='Conv3d_5c_1b_3x3_Trainable')

    branch_2 = conv3d_bn(x, 48, 1, 1, 1, padding='same', name='Conv3d_5c_2a_1x1_Trainable')
    branch_2 = conv3d_bn(branch_2, 128, 3, 3, 3, padding='same', name='Conv3d_5c_2b_3x3_Trainable')

    branch_3 = MaxPooling3D((3, 3, 3), strides=(1, 1, 1), padding='same', name='MaxPool2d_5c_3a_3x3_Trainable')(x)
    branch_3 = conv3d_bn(branch_3, 128, 1, 1, 1, padding='same', name='Conv3d_5c_3b_1x1_Trainable')
    ...
    if include_top:
        ...
        x = GlobalAveragePooling3D( name='global_avg_pool_Trainable')(x)

        x = Dense(100,activation='relu',name='Dense3_Trainable')(x)

        x = Dense(13,activation='linear',name='Dense4_Trainable')(x)

        x = Activation('softmax', name='prediction_Trainable')(x) # This Layer will be removed in the tester
    ...
    for layer in model.layers:
        if('Trainable' in layer.name.split('_')):
...
getpath='/media/aravind/My Drive/Kinetics_Small data/Copying'


folder_path=getpath+'/rgb/'


classlist=sorted(os.listdir(folder_path))


classdict={i:classlist[i] for i in range(0,len(classlist))}


def getminibatch_balanced(getpath,classlist,classdict,shape_v=(79,224,224,3)):
    batchsize=1
    num_classes=len(classlist)
    classfiles=[ os.listdir(folder_path+'/'+classlist[i]) for i in range(0,num_classes)]

    classfiles_copy=[l.copy() for l in classfiles ]

    for lis in classfiles_copy:
        random.shuffle(lis)

    #classfilescount=[len(classfiles[i]) for i in range(0,num_classes)]

    index=random.randint(0,num_classes-1)
    while(1):
        y=np.zeros((batchsize,num_classes))
        for numbatch in range(0,batchsize):
            #print(index)
            class_fol=classdict[index]
            #index=(index+1)%num_classes
            file=classfiles_copy[index].pop()
            if(not classfiles_copy[index]):
                classfiles_copy[index]=classfiles[index].copy()
                random.shuffle(classfiles_copy[index])

            print(getpath+class_fol+'/'+file)
            data=np.load(getpath+class_fol+'/'+file)
            data_shape=data.shape
            newshape=(batchsize,data_shape[0],data_shape[1],data_shape[2],data_shape[3])
            x=data.reshape(newshape)
            y[numbatch,index]=1
            index=(index+1)%num_classes
            yield x,y

gen_data=getminibatch_balanced(folder_path,classlist,classdict)
...
for i in range(0,num_steps):
    print('****Step',i+1,'****')
    x,y=next(gen_data)
    rgb_model.fit(x,y,epochs=1)
    if(i%20000==0):
        now=datetime.datetime.now()

        timestamp=str(now)

        filename='_NotDone_'+timestamp+'_rgb.h5'

        print('Saving RGB model as ',filename,' now......')

        rgb_model.save('./models/'+filename)
